 ...
 *
 * @ingroup ldap_authentication
 *
 * @ContentEntityType(
 *   id = "ldap_authentication",
 *   label = @Translation("Authentication entity"),
 *   handlers = {
 *     "view_builder" = "Drupal\Core\Entity\EntityViewBuilder",
 *     "form" = {
 *       "edit" = "\Drupal\ldap_authentication\Form\LdapAuthenticationAdminForm",
 *     },
 *   },
 *   admin_permission = "administer ldap_authentication module",
 *   fieldable = FALSE,
 *   entity_keys = {
 *     "id" = "sid",
 *     "uuid" = "uuid",
 ...
 *   },
 *   links = {
 *     "edit-form" = "/ldap/authentication/edit/{ldap_server}",
 *   },
 * )
 *
 */
class LdapAuthentication extends ContentEntityBase {
 ...
 */
  ...
  function userUsernameToLdapNameTransform($drupal_username, &$watchdog_tokens) {
    ...
    \Drupal::moduleHandler()->alter('ldap_servers_username_to_ldapname', $ldap_username, $drupal_username, $context);
  ...
  */
  ...
  function userUserNameToExistingLdapEntry($drupal_user_name, $ldap_context = NULL) {
    ...
    foreach ($this->basedn as $basedn) {
      ...
      if ($result['count'] != 1) {
        $count = $result['count'];
        \Drupal::logger('ldap_servers')->error("Error: !count users found with $filter under $basedn.", array('!count' => $count));
        continue;
      }
      $match = $result[0];
 ...
 /**
  ...
  public function groupMembershipsFromUser($user, $return = 'group_dns', $nested = NULL) {
    ...
    elseif ($this->groupGroupEntryMembershipsConfigured) {
      ...
      \Drupal::logger('ldap_servers')->warning('groupMembershipsFromUser: Group memberships for server have not been configured.', array());
      return FALSE;
    }
    if ($return == 'group_dns') {
      return $group_dns;
  ...
  public function groupMembershipsFromEntryRecursive($current_group_entries, &$all_group_dns, &$tested_group_ids, $level, $max_levels) {
	...
	if(!$member_id) {
	  if ($this->detailed_watchdog_log) {
	     \Drupal::logger('ldap_server')->notice('group_entry: %ge', array('%ge'=>pretty_print_ldap_entry($group_entry)));
	  }
	  ...
	  if(is_array($goc)) {              // TODO is it always an array?
	    foreach($goc as $g) {
	      $g=\Drupal\Component\Utility\Unicode::strtolower($g);
	      if($g == $this->groupObjectClass) {
		// found a group, current user must be member in it - so:
		if ($this->detailed_watchdog_log) {
		  \Drupal::logger('ldap_server')->notice('adding %mi', array('%mi'=>$member_id));
		}
		$member_id=$group_entry['dn'];
		break;
	      }
